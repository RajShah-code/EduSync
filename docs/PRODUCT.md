...
9. Admin console — classroom/student/teacher/log management; transient roll-based password generation exposed only at creation, never stored in plaintext. **Requirements not yet fully gathered** for the console's overall ambition — scope beyond basic CRUD is still an open question — but a concrete piece shipped Aug 25–26, 2026: a **Subject Catalog** (reusable subject list) and **Subject Allotments** (class + subject + semester + teacher assignment) admin feature, which also auto-syncs into EduSync Connect classrooms — see "EduSync Connect" below.
...
**Core unit: the "classroom."** One `connect_class_subjects` row = one (teacher, class, subject) allotment. A teacher's classroom list is their own allotments; a student's is every allotment matching their single `class_id`. Same class+subject taught by 2+ teachers, or 2+ subjects by the same teacher to the same class, are disambiguated automatically in the display name (`"Subject(TeacherName)"` / `"ClassName(Subject)"`) rather than shown as indistinguishable duplicates. **Two entry points create these rows, both still live**: Connect's own admin page (`Connect-Frontend/pages/admin/AdminAllotments.jsx`, free-text subject name, admin-provisioned directly), and — added Aug 26, 2026 — automatic one-way sync from EduSync's own Subject Catalog + Subject Allotments admin feature (see `PRODUCT.md`'s Capabilities item 9 and the PRD's §11.5) whenever a curriculum allotment gets a teacher assigned. Deleting/unassigning a synced allotment archives (never deletes) the classroom.

**Features:**
1. **Classroom messaging** — real-time chat scoped to one classroom, Socket.io (`connect:classroom:{id}` room) with a REST fallback send path. **Every classroom is teacher/admin-post-only.** An earlier `posting_mode` toggle (`teacher_only`/`open`, letting a teacher allow students to post) was built and then **removed by product decision (Aug 25, 2026)** — students now read and vote in polls, never post.
...
7. **Live tab updates + Web Push notifications** *(Aug 25–26, 2026)* — Announcements, Assignments, and Materials tabs now push live over the same Socket.io room (previously refresh-only; messages/polls were already live). Separately, a VAPID-based Web Push pipeline notifies students of new messages/announcements/assignments/grading even with the browser closed, and notifies the owning teacher(s) too when the announcement is admin-authored. Push degrades to a safe no-op (not a crash) if VAPID keys aren't configured in a given environment — see `SETUP.md` for the env vars.
...
No case studies, testimonials, press, or benchmark data exist. Do not fabricate any. The project's own PRD (`PRD_Smart_Teaching_Lab_Management_Platform.md`) is the primary source of product truth for EduSync; this document was refreshed against a full codebase read (both `Frontend/` and the new `Connect-Frontend/`) as of Aug 25, 2026, alongside the PRD's own v1.4 update covering EduSync Connect, and re-synced Aug 26, 2026 against the PRD's v1.5 update (open-discussion mode removed, Subject Catalog + Allotments admin feature, live tab updates, Web Push notifications).
