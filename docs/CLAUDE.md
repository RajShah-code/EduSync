...
- **Icons**: `Frontend/` uses **`@tabler/icons-react` only** (`lucide-react`, `@phosphor-icons/react`, and MUI were removed). Exports are `Icon<Name>`, and every import is locally aliased to a lucide-style name (`import { IconTrash as Trash2 } from "@tabler/icons-react"`) so call sites read `<Trash2 />`; keep that aliasing convention when adding icons. `Connect-Frontend/` is a **separate** app that still uses `lucide-react` — don't unify them.
